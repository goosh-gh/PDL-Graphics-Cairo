...
sub new {
    ...
    # REVIEW NOTE (2026-06-22, look-at方式への変更 — 3回目の設計変更):
    #
    # 経緯: theta/phi(オイラー角2軸) → 累積回転行列(トラックボール) →
    # look-at(視線方向+up方向から毎フレーム再構成) という順で設計を
    # 見直した。トラックボール方式(累積回転行列)は、Cz(頭頂)を画面上
    # 任意位置に持っていけるようになった一方、ドラッグの経路によって
    # 「Neurosurgeon's view」として正しいはずの視点(T3が画面右、T4が
    # 画面左になる、放射線科的view "Radiologist's view" とは180度逆)
    # が、同じ見た目の視点でも経路によってT3/T4の左右が反転する場合が
    # あることが実機検証で判明した(rot行列の1行目、つまり画面right方向
    # の基底ベクトルが、ドラッグの累積によって元のX軸からほぼ反転する
    # ところまでズレていた)。
    #
    # look-at方式(OpenGLのgluLookAtと同じ考え方)に変更することで、
    # 状態を「カメラの球面座標(azimuth, elevation)+距離」という、常に
    # 物理的に意味のある値に保ち、毎フレーム
    #   view  = 原点から見たカメラ方向の単位ベクトル
    #   up    = (0,1,0)(Fp方向)をviewに直交化したもの(Gram-Schmidt)
    #   right = up × view  (2026-06-22、2回目の訂正: 当初view×upを
    #           採用したが、goosh氏の確定要件「Czが上に凸(浮く)なら
    #           T3が画面左・T4が画面右、Czが下に凸(沈む)ならT3が画面
    #           右・T4が画面左」という対応をcheck_float_lr_consistency.pl
    #           で全elevation角度にわたって検証した結果、view×upでは
    #           ほぼ全件不一致だった。外積を反転(up×view)することで
    #           この対応がほぼ全件一致することを確認済み。)
    #   rot   = [right; up; view] (3x3、各行が新しい基底)
    # として再構成する。これにより「経路依存」の問題が原理的に起きない
    # (azimuth/elevationの値だけで視点が一意に決まる)。
    #
    # cam_azimuth/cam_elevationは、電極シーンのbearing/elevationと同じ
    # 球面座標の意味(azimuth=0で前方Fp方向から見る、elevation=0で
    # 真上Cz方向から見る)。
    $self->{cam_azimuth}   = $args{cam_azimuth}   // 0.6;
    $self->{cam_elevation} = $args{cam_elevation} // 1.17;  # Phase1初期視点に近い、頭の斜め上から見る角度
    ...
    # (cam_azimuth/cam_elevation と _compute_lookat_rotation は履歴として
    #  残すが、もう種づけには使わない。)
    $self->{qrot} = [ 0, 0, 0, 1 ];
...
# ============================================================
# look-at方式での回転行列の構成(2026-06-22導入)。
#
# $self->{cam_azimuth}/{cam_elevation}(球面座標)から、視線方向(view)・
# 画面上方向(up)・画面右方向(right)を求め、それらを行とする3x3行列を
# 返す。OpenGLのgluLookAtと同じ考え方(view, up, right=up×viewの直交基底
# でワールド座標→カメラ座標の変換行列を作る)。
#
# goosh氏の確定要件(2026-06-22):
#   Czが上に凸(浮く、頭を上から見下ろすNeurosurgeon's view)
#     <=> T3(被験者の左)が画面左、T4(被験者の右)が画面右
#   Czが下に凸(沈む、足元から見上げるRadiologist's view)
#     <=> T3が画面右、T4が画面左
#   (視点の向きによってどちらも正しく、片方が常に誤りということはない。
#    重要なのは「Czの浮き沈み」と「T3/T4の左右」の対応が常に一貫して
#    いること。)
#
# この対応関係を check_float_lr_consistency.pl で全elevation角度に
# わたって実機ロジック(このメソッドそのもの)で検証した結果:
#   right = view × up  だと、ほぼ全角度で不一致
#   right = up × view  だと、ほぼ全角度で一致(境界elevation≈90度付近の
#                       わずかな数値誤差を除く)
# という結果が出たため、right = up × view を採用する。
#
# up方向は常に(0,1,0)(電極シーンのFp方向、bearing=0)を基準にして
# view方向へ直交化(Gram-Schmidt)する。これにより「Fpが前方(画面上の
# どちらかの端)にある」という制約を自動的に満たす。
# ============================================================
sub _compute_lookat_rotation {
    my ($self) = @_;
    my $az = $self->{cam_azimuth};
    my $el = $self->{cam_elevation};

    # カメラ位置(原点からの方向、電極シーンと同じ球面座標規約:
    # x=sin(el)*sin(az), y=sin(el)*cos(az), z=cos(el))。
    # view(視線方向)はカメラ位置から原点(注視点)を見る単位ベクトルで、
    # これは「原点からカメラ位置への方向」と同じ向き(カメラが外側に
    # あり、内側の原点を見ているため)。
    my $vx = sin($el) * sin($az);
    my $vy = sin($el) * cos($az);
    my $vz = cos($el);
    my $vlen = sqrt($vx*$vx + $vy*$vy + $vz*$vz) || 1;
    $vx /= $vlen; $vy /= $vlen; $vz /= $vlen;

    # up方向: (0,1,0)(Fp方向)をviewに直交化(Gram-Schmidt)。
    # view方向がほぼ(0,1,0)に一致する(elevation→0かつazimuth→0)場合、
    # 直交化の結果がゼロベクトルに近くなる退化(ジンバルロック的な)
    # ケースがある。これは「真正面からFp方向を見ている」極端な視点で、
    # 実用上は起こりにくいが、ゼロ除算を避けるため最低限のガードを入れる。
    my $fwd_x = 0; my $fwd_y = 1; my $fwd_z = 0;
    my $dot = $fwd_x*$vx + $fwd_y*$vy + $fwd_z*$vz;
    my $ux = $fwd_x - $dot*$vx;
    my $uy = $fwd_y - $dot*$vy;
    my $uz = $fwd_z - $dot*$vz;
    my $ulen = sqrt($ux*$ux + $uy*$uy + $uz*$uz);
    if ($ulen < 1e-6) {
        # 退化ケース: view方向に対して別の基準ベクトル(0,0,1)で代用
        $fwd_x = 0; $fwd_y = 0; $fwd_z = 1;
        $dot = $fwd_x*$vx + $fwd_y*$vy + $fwd_z*$vz;
        $ux = $fwd_x - $dot*$vx;
        $uy = $fwd_y - $dot*$vy;
        $uz = $fwd_z - $dot*$vz;
        $ulen = sqrt($ux*$ux + $uy*$uy + $uz*$uz) || 1;
    }
    $ux /= $ulen; $uy /= $ulen; $uz /= $ulen;

    # right = up × view
    #
    # REVIEW NOTE (2026-06-22, 2回目の訂正): 当初 view×up をNeurosurgeon's
    # viewの規約として採用したが、これは「Czの浮き沈み」と「T3/T4の
    # 左右」の対応(goosh氏確定要件: Cz上に凸<=>T3左、Cz下に凸<=>T3右)を
    # 検証した結果、ほぼ全ケースで不一致だった(check_float_lr_consistency.pl
    # で実機ロジックそのものを使って確認)。外積の順序を反転(up×view)
    # することで、この対応が一致するかを再検証する。
    my $rx = $uy*$vz - $uz*$vy;
    my $ry = $uz*$vx - $ux*$vz;
    my $rz = $ux*$vy - $uy*$vx;
...
$self->{qrot}に累積し、$self->{rot}はそこから導出。左右対応はlook-at既定
姿勢を_mat_to_quatで初期クォータニオンに種づけして無損失に継承。極が無く、
